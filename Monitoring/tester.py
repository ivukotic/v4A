...
def load_configurations():
    ...
    if not isinstance(data, object):
        raise ValueError(
            "Expected a top-level JSON object, got something else!")
...
def get_es_client():
    es = Elasticsearch("https://atlas-kibana.mwt2.org:9200",
...
def test_endpoint(site) -> bool:
    try:
        response = requests.get(
            f'http://{site["url"]}:{site["port"]}/atlr', timeout=10, headers=headers)
