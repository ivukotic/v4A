...
while true; do
  ...
  jsn=$(echo $fdatb | jq --arg INST "$INSTANCE" --arg SITE "$SITE" '. += { kind: "cvmfs", instance: $INST, site: $SITE }')
  ...
  ajs=$(echo "{\"kind\":\"cvmfs\",\"instance\":\"$INSTANCE\",\"site\":\"$SITE\"}" | jq | jq --argjson CNT "$cnt" '. +={ cnt: $CNT }')
